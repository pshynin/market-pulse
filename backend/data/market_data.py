...
class MarketDataCollector:
    ...
    def collect_all_data(self) -> Optional[MarketData]:
        ...
        try:
            ...
            logger.info(f"Market data collected and saved (ID: {market_data.id})")
            return market_data
    ...
    def get_stock_price(self, symbol: str) -> float:
        ...
        if not self.alpha_vantage_key:
            logger.warning(f"Alpha Vantage API key not set. Using sample data for {symbol}.")
        ...
        try:
            ...
            if "Global Quote" in data and "05. price" in data["Global Quote"]:
                ...
                logger.warning(f"Could not fetch price for {symbol}. Using sample data.")
                sample_prices = {"SPY": 475.23, "RSP": 152.87}
                return sample_prices.get(symbol, 100.0)
    ...
    def get_vix_value(self) -> float:
        ...
        try:
            if self.alpha_vantage_key:
                # Could use Alpha Vantage to get VIX data, but using sample for now
    ...
    def get_liquidity_metrics(self) -> Tuple[float, float]:
        ...
        # For this example, using sample data
        try:
            return 21500.0, 7500.0  # Sample M2 supply (billions) and Fed balance sheet (billions)
        except Exception as e:
